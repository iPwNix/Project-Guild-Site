	...
	<div class="container margin-top">
	    ...
	    <div class="row">
	        ...
	        <div class="col-md-12">
			                		...
			                		<div class="chatter_avatar">
					        			@if(Config::get('chatter.user.avatar_image_database_field'))
					        				...
					        				@if( (substr($post->user->{$db_field}, 0, 7) == 'http://') || (substr($post->user->{$db_field}, 0, 8) == 'https://') )
					        					<img src="/uploads/avatars/{{ $post->user->{$db_field}  }}">
					        				@else
					        					<img src="/uploads/avatars/{{ Config::get('chatter.user.relative_url_to_image_assets') . $post->user->{$db_field}  }}">
					        				@endif
	            ...
	            @if(!Auth::guest() && Auth::user()->siteRole !== "1" || Auth::user()->siteRole !== "8")
	            	...
	            	<div id="new_response">
	            		...
	            		<div class="chatter_avatar">
		        			@if(Config::get('chatter.user.avatar_image_database_field'))
		        				...
		        				@if( (substr(Auth::user()->{$db_field}, 0, 7) == 'http://') || (substr(Auth::user()->{$db_field}, 0, 8) == 'https://') )
		        					<img src="/uploads/avatars/{{ Auth::user()->{$db_field}  }}">
		        				@else
		        					<img src="/uploads/avatars/{{ Config::get('chatter.user.relative_url_to_image_assets') . Auth::user()->{$db_field}  }}">
		        				@endif
			            ...
			            <div id="new_discussion">
				            ...
				            <form id="chatter_form_editor" action="/{{ Config::get('chatter.routes.home') }}/posts" method="POST">
						        ...
						        <!-- BODY -->
						    	<div id="editor">
									<label id="tinymce_placeholder">Add the content for your reply here</label>
					...
					@if(Auth::user()->siteRole == "5" ||
                        ...
                        Auth::user()->siteRole == "7")
					   <div class="col-xs-12">
	                    <form action="{{ url('/forums/lock') }}/{{$discussion->id}}" method="POST">
				            {{ csrf_field() }}
				            {{ method_field('PATCH') }}

				            <button class="btn btn-full unlock-btn">Lock</button>
				        </form>
	                  </div>
                  @endif
					<!-- IF LOCKED -->
					@else
						<div class="col-xs-12 locked-forum-div">
							<span><!-- {{$discussion->id}} -->
								LOCKED
							</span>
						</div> 
					@if(Auth::user()->siteRole == "5" ||
                        ...
                        Auth::user()->siteRole == "7")
                        <div class="col-xs-12">
                    <form action="{{ url('/forums/unlock') }}/{{$discussion->id}}" method="POST">
			            {{ csrf_field() }}
			            {{ method_field('PATCH') }}

			            <button class="btn btn-full unlock-btn">Unlock</button>
			        </form>
                        </div>
					@endif
				...
				@else
					...
					@else
					<div id="login_or_register">
						<p>Please wait for your Application to be approved</p>
					</div>
					@endif
				@endif
