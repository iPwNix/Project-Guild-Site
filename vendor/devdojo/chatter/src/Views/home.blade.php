...
@if(!Auth::guest() && Auth::user()->siteRole !== "1" && Auth::user()->siteRole !== "8" && Auth::user()->siteRole !== "9")
...
<div id="chatter" class="chatter_home">
	...
	<div class="container chatter_container">
	    ...
	    <div class="row">
	    	...
	    	<div class="col-md-3 left-column col-forum-navi">
	    		<!-- SIDEBAR -->
	    		<div class="chatter_sidebar">
					<button class="btn btn-primary" id="new_discussion_btn"><i class="chatter-new"></i> New Thread</button> 
					<a href="/{{ Config::get('chatter.routes.home') }}" class="forum-home-nav-btn"><i class="chatter-bubble"></i> All {{ Config::get('chatter.titles.discussion') }}</a>
					<ul class="nav nav-pills nav-stacked nav-forum-side">
						<?php $categories = DevDojo\Chatter\Models\Category::all(); ?>
						@foreach($categories as $category)

							@if($category->name === "GMC")
	        ...
	        <div class="col-md-9 right-column">
	        	<div class="panel main-forum-panel">

		        	<ul class="discussions">
		        		@foreach($discussions as $discussion)
		        		@if($discussion->category->name !== "GMC")
				        	<li>
				        		<a class="discussion_list main-discussion-list" href="/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.discussion') }}/{{ $discussion->category->slug }}/{{ $discussion->slug }}">
					        		<div class="chatter_avatar">
					        			@if(Config::get('chatter.user.avatar_image_database_field'))
					        				...
					        				@if( (substr($discussion->user->{$db_field}, 0, 7) == 'http://') || (substr($discussion->user->{$db_field}, 0, 8) == 'https://') )
					        					<img src="/uploads/avatars/{{ $discussion->user->{$db_field}  }}">
					        				@else
					        					<img src="/uploads/avatars/{{ Config::get('chatter.user.relative_url_to_image_assets') . $discussion->user->{$db_field}  }}">
					        				@endif
					        		...
					        		<div class="chatter_right">
					        			...
					        			@else
					        			<div class="chatter_count"><i class="fa fa-lock" aria-hidden="true"></i></div>
					        			@endif
					        		</div>
				        	...
				        	@if(Request::url() === 'http://aegisofenvy.com/forums/category/gmc')
				        	<li>
				        		<a class="discussion_list main-discussion-list" href="/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.discussion') }}/{{ $discussion->category->slug }}/{{ $discussion->slug }}">
					        		<div class="chatter_avatar">
					        			@if(Config::get('chatter.user.avatar_image_database_field'))
...
<!-- 	        	<div id="pagination">
	        	...
	        	{!! with(new App\Pagination\HDPresenter($discussions))->render(); !!}
	...
	<div id="new_discussion">
    	...
    	<form id="chatter_form_editor" action="/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.discussion') }}" method="POST">
            ...
            <!-- BODY -->
        	<div id="editor">
				<label id="tinymce_placeholder">Add the content for your Thread here</label>
